...
const JobCard = ({ job, onClick, onToggleSave, onApply, saved, hideApply }) => {
    ...
    const formatSalary = (min, max) => {
        ...
        return `$${formatNumber(min)}/m`; // Note: Your code currently returns just the min value formatted.
    };

    return (
        <div className="bg-white rounded-2xl border border-gray-200 p-6 hover:shadow-xl hover:shadow-gray-200 transition-all duration-300 cursor-pointer relative overflow-hidden"
            onClick={onClick}
        >
                ...
                {!saved && (
                    <>
                        {job?.applicationStatus ? (
                            ...
                            !hideApply && (
                                <button
                                    className="bg-gradient-to-r from-blue-50 to-blue-50 text-sm text-blue-700 hover:text-white hover:from-blue-500 hover:to-blue-600 transition-all duration-200 px-6 py-2.5 rounded-xl font-semibold transform hover:-translate-y-0.5"
                                    onClick={(e) => {
                                        e.stopPropagation();
                                        onApply();
                                    }}
